...
#sys.path.reverse()
    ...
    #   Earthquake Methods library of plot methods and functions
    #   
...
def map_swarm_epicenters_all(burst_min_size, completeness_mag, Location, N_Steps, \
...
def map_swarm_epicenters_one_burst(completeness_mag, Location, N_Steps, \
    ...
    mag_array, date_array, time_array, year_array, depth_array, lat_array, lng_array = BSTFileMethods.read_circle_catalog()
    ...
    for i in range(len(burst_index)):        #   Iterating over the bursts/swarms
        ...
        if len(burst_index[i]) >= burst_min_size:
            ...
            radius_gyration, burst_centroid_lat, burst_centroid_lng = \
                    BSTCalcMethods.compute_burst_radius_gyration(burst_index[i], mag, lat, lng, date, time, years)
...
#   Filter the clusters now, we filtered the events in the clusters previously
    ...
    radgyr_list, year_swarm, index_swarm, centroid_lat, centroid_lng = \
            BSTCalcMethods.reject_outliers_clusters_alternate(ratio_limit, burst_index, burst_min_size, mag, lat, lng, date, time, years)
...
#         RG_list_raw = [math.log(i,10) for i in RG_list_raw]
        ...
        radgyr_EMA = BSTCalcMethods.EMA_weighted_time_series(RG_list_raw, N_Steps)
...
#
    os.remove('interpolation_file.txt')
    ...
    for n in range(len(ENF_list)):
        ...
        for k in range(len(CLF_list)):
            ...
            year_swarm_temp,radgyr_swarm_temp = \
                    BSTCalcMethods.calc_radgyr_EMA_time_dev(burst_min_size, completeness_mag, Location, N_Steps,\
                    circle_catalog_date_start, circle_catalog_date_end, burst_index, mag, lat, lng, date, time, years, \
                    ratio_limit, sd_factor, cutoff_start_year, regional_rate)
...
#   -----------------------------------------------------------------
    ...
    for i in range(len(radgyr_swarm_regular)):
        ...
        mean_ts, std_ts = BSTCalcMethods.time_series_mean_std(values_list, adjusted_weights)
...
#   Determine values of radgyr at the time the large M>7 earthquakes occur
    ...
    print('------------------------------------------------')
    print('')
    print('Date of M>7 EQ: ', earthquake_date_M7)
    print('')
    print('Year of M>7 EQ: ', earthquake_year_M7)
    print('')
    
    year_M7_EQ = [round(year_M7_EQ[i],5) for i in range(len(year_M7_EQ))]
...
#     mag_array, date_array, time_array, year_array, depth_array, lat_array, lng_array = read_circle_catalog()
    ...
    for i in range(len(burst_index)):
        index_list = burst_index[i]
        radius_gyration, burst_centroid_lat, burst_centroid_lng = \
            BSTCalcMethods.compute_burst_radius_gyration(index_list, mag, lat, lng, date, time, years)
